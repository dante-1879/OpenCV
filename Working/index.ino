...
int lastStateValues[5] = {0, 90, 90, 180, 180};  // Pinky and ring start closed (flipped)
      ...
      if (finger >= 0 && finger < 5) {
        if (servoTypes[finger]) {
          ...
          if (commandType == "state" && value != lastStateValues[finger]) {
            ...
            if (value == 0) {
              // Open state
              moveAngle = (finger == 3 || finger == 4) ? 150 : 30;  // Flipped for pinky and ring
              Serial.print("Finger ");
              Serial.print(finger);
              Serial.println(" (state-based) -> OPEN");
            } else if (value == 180) {
              // Closed state
              moveAngle = (finger == 3 || finger == 4) ? 30 : 150;  // Flipped for pinky and ring
              Serial.print("Finger ");
              Serial.print(finger);
